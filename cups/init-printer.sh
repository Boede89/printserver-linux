...
# Nur wenn CUPS_AUTO_PRINTER=true — sonst startet CUPS erst einmal leer.
set -e
...
case "$(echo "$AUTO" | tr '[:upper:]' '[:lower:]')" in
  1|true|yes|on) ;;
  *)
    echo "[cups-init] CUPS_AUTO_PRINTER nicht aktiv — kein Drucker (CUPS startet leer)."
    mkdir -p /etc/cups
    if [ ! -f "$CONF" ]; then
      echo "# Kein Drucker — siehe docs/DRUCKER-KONICA-MINOLTA.md" > "$CONF"
    fi
    exit 0
    ;;
esac
...
if [ -z "$PRINTER_URI" ]; then
  echo "[cups-init] CUPS_PRINTER_URI fehlt — überspringe."
  exit 0
fi
